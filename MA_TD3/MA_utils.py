...
def mini_batch_train(env: object, agent: object, env_name: str, max_episodes: int, max_steps: int, batch_size: int)-> float:
    ...
    for episode in range(max_episodes):
        ...
        reward = 0
        old_action = np.zeros([action_dim])
        ...
        while not done :
            ...
            if total_steps < 1000:
                ...
                state_list = []
                # state, old_action list 생성
                for _ in range(action_dim):
                    state_list.append(np.concatenate([state,old_action[1:]],axis=0))
                    old_action = np.insert(old_action, action_dim,old_action[0])
                    old_action = np.delete(old_action,0)
            else:
                # information will choose which node we will make null
                information = np.array([1, 1, 1, 1, 1, 1])                
                if random.random() < 0.05:
                    information[np.random.randint(0,6)] = 0 

                # 5는 몇번 Loop를 돌것인지를 의미합니다. Closed Loop feed back way
                for _ in range(5):                
                    state_list = []                    
                    for _ in range(action_dim):
                        state_list.append(np.concatenate([state,old_action[1:]],axis=0))
                        old_action = np.insert(old_action, action_dim,old_action[0])
                        old_action = np.delete(old_action,0)
                    action, action_ = agent.get_action(state_list)
                    action *=  information
            ...
            # Make the list for next_state, old_action for replay buffer
            for _ in range(action_dim):
                next_state_list.append(np.concatenate([next_state,old_action[1:]],axis=0))
                old_action = np.insert(old_action, action_dim,old_action[0])
                old_action = np.delete(old_action,0)            

            # push to the replay buffer
            agent.replay_buffer.push(state_list, action, reward, next_state_list, done)
            ...
            if len(agent.replay_buffer) > batch_size:
                agent.update(batch_size, total_steps)               
...
def evaluate(env_name: str, agent: object, WORKSPACE: str, episode: int, total_steps: int, start_time: float):
    ...
    for eval_episode in range(5):
        ...
        while not eval_done :
            eval_state_list = []
            for _ in range(action_dim):
                eval_state_list.append(np.concatenate([eval_state,old_action[1:]],axis=0))
                old_action = np.insert(old_action, action_dim,old_action[0])
                old_action = np.delete(old_action,0)
            eval_action, eval_action_= agent.get_action(eval_state_list)
            eval_next_state, eval_reward, eval_done, _ = eval_env.step(eval_action_)                         
