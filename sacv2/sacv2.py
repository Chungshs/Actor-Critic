...
class SACAgent():
    def __init__(self, env: object, gamma: float, tau: float, buffer_maxlen: int,
     critic_lr: float, actor_lr:float, reward_scale: int, alpha_lr: float):
    ...
    def update(self, batch_size: int):
        ...
        # Backpropagate gradient to update the alpha 
        self.alpha_optim.zero_grad()
        alpha_loss.backward()
        self.alpha_optim.step()

        # Convert log alpha to alpha using exponetial
        self.alpha = self.log_alpha.exp()

    def get_action(self, state: np.ndarray, stochastic: bool):
